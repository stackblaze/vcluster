...
func AddHelmFlags(cmd *cobra.Command, options *cli.DeleteOptions) {
	...
	cmd.Flags().BoolVar(&options.DeleteDatabase, "delete-database", false, "If enabled, vcluster will delete the external database and user created by the connector (PostgreSQL/MySQL)")
}
